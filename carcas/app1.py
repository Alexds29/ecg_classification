...
model = models.load_model('neuron_net_clf.h5')
...
def enter_item():
 ...
 #           v = np.array(list(pairs.values())).reshape(1, -1)
            ...
            print (v)
            if np.argmax(model.predict(v), axis=-1) == 0:
                flash('Sinus Bradycardia (Синусовая брадикардия)')
            elif np.argmax(model.predict(v), axis=-1) == 1:
                flash('Sinus Rhythm (Синусовый ритм)')
            elif np.argmax(model.predict(v), axis=-1) == 2:
                flash('Atrial Fibrillation (Мерцательная аритмия)')
            elif np.argmax(model.predict(v), axis=-1) == 3:
                flash('Sinus Tachycardia (Синусовая тахикардия)')
            elif np.argmax(model.predict(v), axis=-1) == 4:
                flash('Supraventricular Tachycardia (Наджелудочковая тахикардия)')
            elif np.argmax(model.predict(v), axis=-1) == 5:
                flash('Atrial Flutter (Трепетание предсердий)')
            elif np.argmax(model.predict(v), axis=-1) == 6:
                flash('Sinus Atrium (Синус предсердия)')
            elif np.argmax(model.predict(v), axis=-1) == 7:
                flash('Atrial Tachycardia (Предсердная тахикардия)')
            else:
                flash('Такого диагноза не существует')
